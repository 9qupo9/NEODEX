...
import "dex/internal/ui/walletconnect"
...
func RenderHeader(activeMenu string) string {
	...
	case "futures":
		futuresClass = ` class="active"`
	}

	marketSelectorHTML := ""
	if activeMenu == "spot" || activeMenu == "futures" {
		marketSelectorHTML = `
	...
	<header class="header">
		<div class="header-left">
			...
			<nav class="nav-links">
				...
				<a href="/futures"` + futuresClass + `>Futures</a>
				` + marketSelectorHTML + `
			</nav>
		</div>
		<div class="header-right">
			` + walletconnect.NewWalletConnectService().GetButtonHTML() + `
